...
C
C        INPUT
C         ARRAY VALUES (COMWT) - TESAC REPORTS
C         L31  (COMWT)  -  LOGICAL FOR SURFACE REPORT
C         L32  (COMWT)  -  LOGICAL FOR SUB-SURFACE REPORT
C
C        OUTPUT
C         ARRAY VALUES (COMWT) - TESAC REPORTS WITH CONFIDENCE FLAGS
C                                               ADDED
C         IERR   -  ERROR CODE
C
C       METHOD.
...
C
      IF(FF.NE.RMISS) THEN
           ...
           IF(IERR.NE.0) WRITE (*,'(1H ,''TESAC CSLT ERROR:'',I5,
     X                '' FF:'',F10.2)') IERR,FF
      ENDIF
C
