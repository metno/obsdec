      SUBROUTINE ISFILE8(ctype,ctime,cfname,ret_code)
C
C**** *ISFILE8*
C
C
...
Crr        This is for drau files - name for each 3. hour
C
C**   INTERFACE.
C     ----------
C
C         *CALL* *ISFILE8(CTYPE,CTIME,CFNAME,RET_CODE)*
C
C         INPUT   : CTYPE
C                   CTIME
C         OUTPUT  : CFNAME
...
C
      ...
      IF(IDIFF.LE.0) THEN
         IF(IDIFF.LT.-25) THEN
            PRINT*,' isfile8: WARNING - DATA FOR FUTURE ',CT,
     *           ' diff.mins.=',idiff
            RET_CODE=3
            RETURN
         END IF
...
cpsdec02      IF(IDIFF.GT.7200) THEN           !  5 days delay
      IF(IDIFF.GT.60*24*2) THEN           !  2 days delay
         PRINT*,' isfile8: WARNING - TOO OLD DATE ',CT
         RET_CODE=4
         RETURN
      END IF
...
c
cpsapr98      if(itime.ge.2231.and.itime.le.2359) ctime(7:8)='00'
      if(itime.ge.2231.and.itime.le.2359) ctime(9:10)='00'
      if(itime.ge.0000.and.itime.le.0130) ctime(9:10)='00'
      if(itime.ge.0131.and.itime.le.0430) ctime(9:10)='03'
      if(itime.ge.0431.and.itime.le.0730) ctime(9:10)='06'
      if(itime.ge.0731.and.itime.le.1030) ctime(9:10)='09'
      if(itime.ge.1031.and.itime.le.1330) ctime(9:10)='12'
      if(itime.ge.1331.and.itime.le.1630) ctime(9:10)='15'
      if(itime.ge.1631.and.itime.le.1930) ctime(9:10)='18'
      if(itime.ge.1931.and.itime.le.2230) ctime(9:10)='21'

      if(itime.ge.2231.and.itime.le.2359) then
         id=id+1
